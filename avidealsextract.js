...
function convertExcelDateToJSDate(excelDate) {
  ...
  var days = Math.floor(excelDate);
  var milliseconds = (excelDate - days) * 24 * 60 * 60 * 1000;
  return new Date(baseDate.getTime() + days * 24 * 60 * 60 * 1000 + milliseconds);
}
...
async function example() {
  ...
  // let extractData="genericlinks" //ASIN	Category	MRP	Sale Price	Landing page
  // let extractData="categorylinks"
  let extractData="productlinks"

  let extractType="bestselling"
  // let extractType="others"

  if(env=="prod"){
    // jsonFileName = "bs_Kitchen"
    jsonFileName = "bs_Softlines"
    // jsonFileName = "bs_Major_Appliances"
...
// try{
  if(extractData=="productlinks" && extractType=="bestselling"){
  //  excelFilePath = 'F:/Website/Excel/Top Deals.xlsx';
   excelFilePath = 'C:/Users/avina/Downloads/Telegram Desktop/bestselling.xlsx';
  }
  else if(extractData=="productlinks"){
  //  excelFilePath = 'F:/Website/Excel/Top Deals.xlsx';
   excelFilePath = 'C:/Users/avina/Downloads/Telegram Desktop/Hot-Deals-6.xlsx';
  }
  else if(extractData=="categorylinks"){
    excelFilePath = 'F:/Website/Excel/Top deals and launch announcements-4.xlsx';
  }
  else if(extractData=="genericlinks"){
    excelFilePath = 'C:/Users/avina/Downloads/Telegram Desktop/jewellery-deals.xlsx';
  //  excelFilePath = 'F:/Website/Excel/Top deals and launch announcements-4.xlsx';
  }
...
// }
    ...
    // for (let i = 2; i <= 2501; i++) { // Assuming data starts from the 2nd row
    //     const rowData = xlsx.utils.sheet_to_json(worksheet, { header: 1 })[i];
    //     const startDatetime = rowData[0];
    // const endDatetime = rowData[1];
    // const productFamily = rowData[2];
    // const productCategory = rowData[3];
    // const productSubcategory = rowData[4];
    // const dealType = rowData[5];
    // const dealTitle = rowData[6];
    // const description = rowData[7];
    // const dealLandingPage = rowData[8];
    // const asin = rowData[9];
    // const productName = rowData[10];
    // const productURL = rowData[11];
    // const brand = rowData[12];
    // const dealPrice = rowData[13];
    // const previousPrice = rowData[14];
    // const totalDiscount = rowData[15];
    // const asinRating = rowData[16];
    // const asinReviews = rowData[17];
    // const primeAccessType = rowData[18];  
        
    //     // Perform Selenium automation and get data
    //     // const resultData = await performAutomation(rowData);
    ...
    // // Convert array of JSON objects to JSON string
    // const jsonDataString = JSON.stringify(jsonDataArray, null, 2);
    // console.log(jsonDataString);
...
let keyValueCounts = {};
const inputData = [];
// const inputData = [1, 2, 3, 1, 2, 4, 5, 1, 2, 3];
  ...
  // let extract = "Telegram";
  
  let url = "https://dealshubglobal-default-rtdb.firebaseio.com";

  if(env=="prod"){
    // if(extract == "Excel" && extractData == "productlinks"){
    if(extractData == "productlinks" || extractData== "genericlinks"){
  ...
  let access_token = await getAccessToken(env,extract,extractData);
  ...
  len = await firebaseget(env,jsonFileName,url,extract);
  ...
  let N;
  if(extract == "Telegram" || extract == "Json") {
    N=Input.length;
    i=0;
  } else if(extract == "Excel"){
    N=rowDataCount; // Add 1 for not using = or add 2 if header starts from 2
    i=2;
  }

  for (; i < N; i++) {
    console.log("i is ",i)
    if(Input && Input[i]?.from == undefined){
      // Input[i].from="";
    }
  // for (i = 0; i < Input.length; i++) {
    ...
    if(extractData == "productlinks" && extractType=="bestselling"){
      console.log("Entered into best selling excel productlinks extraction")
      rowData = xlsx.utils.sheet_to_json(worksheet, { header: 1 })[i];
      urlasin = String(rowData[9]);
      // urlasin = String(rowData[6]);
      console.log("asin is ",urlasin)
      console.log("extract path is ",excelFilePath)

    }
    if(extractData == "productlinks" && extractType!="bestselling"){
      console.log("Entered into excel productlinks extraction")
      rowData = xlsx.utils.sheet_to_json(worksheet, { header: 1 })[i];
      startDatetime = String(rowData[0]);
      endDatetime = String(rowData[1]);
      productFamily = rowData[2];
      productCategory = rowData[3];
      productSubcategory = rowData[4];
      dealType = rowData[5];
      dealTitle = rowData[6];
      description = rowData[7];
      dealLandingPage = rowData[8];
      asinData = rowData[9];
      productName = rowData[10];
      productURL = rowData[11];
      brand = rowData[12];
      dealPrice = rowData[13];
      previousPrice = rowData[14];
      totalDiscount = rowData[15];
      asinRating = rowData[16];
      asinReviews = rowData[17];
      primeAccessType = rowData[18];  

      console.log(startDatetime); // Output: Sat Dec 15 2023 08:30:00 GMT+0000 (Coordinated Universal Time)
      console.log(endDatetime);   // Output: Sun Dec 31 2023 23:30:00 GMT+0000 (Coordinated Universal Time)
      // if(startDatetime.includes(" ")){
      if(extractData=="productlinks"){
        startDatetime = convertExcelDateToJSDate(startDatetime);
        endDatetime = convertExcelDateToJSDate(endDatetime);
      }
      else{
        console.log("date without spaces",startDatetime)
      }

      // Log the results
      console.log("Dates are:"); // Output: Sat Dec 15 2023 08:30:00 GMT+0000 (Coordinated Universal Time)
      console.log(startDatetime); // Output: Sat Dec 15 2023 08:30:00 GMT+0000 (Coordinated Universal Time)
      console.log(endDatetime);   // Output: Sun Dec 31 2023 23:30:00 GMT+0000 (Coordinated Universal Time)
    }
    else if(extract == "Excel" && extractData == "categorylinks"){
      console.log("Entered into excel categorylinks extraction")
      rowData = xlsx.utils.sheet_to_json(worksheet, { header: 1 })[i];
      Category = rowData[0];
      Title = rowData[1];
      PromotionText = rowData[2];
      CategoryUrl = rowData[3];
      StartDate = rowData[4];
      EndDate = rowData[5];
      // Asins = rowData[6];
      ImageLink = rowData[7];
      // ImageLink = row.getCell(7).text;
      console.log("Image Link in categorylinks is ",ImageLink)
    }
    else if(extract == "Excel" && extractData == "genericlinks"){
      console.log("Entered into excel genericlinks extraction")
      rowData = xlsx.utils.sheet_to_json(worksheet, { header: 1 })[i];
      ASIN = rowData[0];
      Category = rowData[1];
      MRP = rowData[2];
      Sale_Price = rowData[3];
      Landing_page = rowData[4];
      // EndDate = rowData[5];
      // Asins = rowData[6];
      // ImageLink = rowData[7];
      // ImageLink = row.getCell(7).text;
      console.log("Landing Link in genericlinks is ",Landing_page)
    }
    if(extract == "Telegram"){
      M=Input[i].text_entities.length;
    }
    else if(extract == "Excel" || extract == "Json" || extract == "File"){
      M=1;
      // j=2;
    }
          ...
          for (j = 0; j < M; j++) {
            ...
            try {
              ...
              if(extract == "Telegram"){
                console.log("Extract in telegram")
              ...
              if (!Input[i].text_entities[j].type.includes("link")) {
                ...
                if (Input && Input[i] && Input[i].from && Input[i].from.includes("All1App")) {
                  if (Input[i].text_entities[j].text.includes("#")) {
                    console.log("skipping in #")
                    //changed on 1-12-22
                    // break;
                  if(extract!="Excel") //high check
                    continue;
                  }
                }
              ...
              if(extractType=="bestselling"){
                console.log("Inside best selling asins")
                Links = `https://amazon.in/dp/${urlasin}`
              }
              
              else if (!(Links.includes("amzn")) || !(Links.includes("amazon"))){
                ...
                // if(true){
                if(extract=="Telegram" && !Input[i].from.includes("All1App")){
                ...
                await driver.findElement(By.id("deallinkshorturl")).getAttribute("value").then(function(webElement) {
                  ...
                  console.log('earnlink1 is '+earnlink1); //high check earnlink1
                }, function(err) {
                  if (err.state && err.state === 'no such element') {
                    console.log('Deal not found');
                  }
                });
                console.log('earn link is '+earnlink1);
              ...
              if((extract=="Telegram" && !(Input[i].from.includes("All1App"))) || extract=="File" || extract=="Excel"){
              ...
              if((Input && Input[i] && Input[i].from && Input[i].from.includes("All1App")) && telegramVisit){
                  telegramVisit = false
                ...
                try {
                  ...
                  if(photo != "" && photo.length <300 && pricenum>0 && earnlink1.length != 0 && earnlink1!=""){
                    ...
                    if(enableFirebasePost){
                      postflag = await firebasepost(app,access_token,env,jsonFileName,url,extract);
                      console.log("Post Flag is ",postflag)
                      // id+=1;
                      if(postflag){
                        id+=1;
                      } else {
                        access_token = await getAccessToken(env,extract,extractData);
                        postflag = await firebasepost(app,access_token,env,jsonFileName,url);
                        console.log("Post Flag is ",postflag)
                        if(postflag){
                          id+=1;
...
i++;
              ...
              if (Input && Input[i] && Input[i].from && Input[i].from.includes("Telugu Tech World")) {
                Links = Input[i].text_entities[j].href;
              }
            }
            else if(extract == "Excel" && extractData == "productlinks" && extractType!="bestselling"){
              console.log("Inside productlinks for Url")
              Links = productURL;
            }
            else if(extract == "Excel" && extractData == "productlinks" && extractType=="bestselling"){
              console.log("Inside best selling asins")
              ...
              console.log("Inside genericlinks for Url")
              Links = Landing_page;
            }
            else if(extract == "Excel" && extractData == "categorylinks"){
              console.log("Inside categorylinks for Excel Url")
              Links = CategoryUrl;
            }
            else if(extract == "Json" && extractData == "categorylinks"){
              console.log("Inside categorylinks for Json Url")
              Links = Input[i].links.CategoryUrl;
            }
            ...
            try{
              ...
              try {
                ...
                if (pricenum == 0) {
                  // break; // To Allow Category products
                }
              } catch (e) {
                console.log("Price Error");
              }
              ...
              try {
                ...
                if (Input && Input[i] && Input[i].from && Input[i].from.includes("All1 deals")) {
              ...
              try {
                if (price != "NaN") {
                  ...
                  if (pricenum != 0) {
                    ...
                    // break; //to allow for category products
                    if(extract=="Excel" && extractData=="productlinks"){
                      // pricenum=dealPrice
                    }
                  }
              ...
              let l,
                count = 0;
              // if(Input && Input[i] && Input[i].from && !Input[i].from.includes('All1App')){ //Only All1App Start
                    ...
                    switch (l + 1) {
                      ...
                      case 2:
                        t1 =
                          ...
                          pricenum +
                          "\n " +
                          `https://dealshubglobal.com/deals/product/${id-1}`;
                      ...
                      case 3:
                        t1 =
                          ...
                          pricenum +
                          "\n " +
                          `https://dealshubglobal.com/offers/product/${id}`;
                      ...
                      case 4:
                        t1 =
                          ...
                          pricenum +
                          "\n " +
                          `https://dealshubglobal.com/avideals/product/${id}`;
    ...
    // if(Input && Input[i] && Input[i].from && Input[i].from!= "all1appdaily"){}
    ...
    try {
      if (Input[i] != undefined)
        if (Input && Input[i] && Input[i].from && Input[i].from != undefined)
          if (Input && Input[i] && Input[i].from && Input[i].from.includes("All1App")) {
            // if(Input[i].text_entities!=undefined)
    ...
    try {
      ...
      // if (Input[i] != undefined)
        ...
        if((extract=="Telegram" && !(Input[i].from.includes("All1App"))) || extract=="File" || extract=="Excel"){
            ...
            else if(Input && Input[i] && Input[i].from && Input[i].from == "All Deals")
            {
              telegram(photo, chatId, text);
            ...
            try{
              if(extract == "Excel" && extractData=="productlinks" && extractType!="bestselling"){
                jsonFileName = productCategory
              }
              if(extract == "Excel" && extractData=="genericlinks"){
                jsonFileName = Category
              }
              else if(extract == "Excel" && extractData=="categorylinks"){
                jsonFileName = Category
            ...
            try {
              if(photo != "" && urlvalue1.length != 0 && urlvalue1!="" && (pricenum > 0 || (extract=="Excel" && (extractData=="productlinks" || extractData == "genericlinks") )) && (disco > 0 || (extract=="Excel" && (extractData=="productlinks" || extractData=="genericlinks") )) ){
                console.log("=====> Entering into Json File change Optimizer")
                if (keyValueCounts.hasOwnProperty(productCategory)) {
                  // If it exists, increment the count
                  keyValueCounts[productCategory]++;
                } else {
                  // If it doesn't exist, add the key with a count of 1
                  // keyValueCounts[productCategory] = 1;
                  len = await firebaseget(env,jsonFileName,url,extract,extractData);
                  keyValueCounts[productCategory] = len+1
                }
              }
              else{
                console.log("================> Not Handling Optimized JsonFileName")
                len = await firebaseget(env,jsonFileName,url,extract,extractData);
                keyValueCounts[productCategory] = len+1
              ...
              // if(extract=="excel" && pricenum<0){
              if(extract == "Json" && extractData=="categorylinks"){
                var app = {
                ...
                "urltext": String(Input[i].urltext),
                ...
                "links": {
                  ...
                  "CategoryUrl": String(Input[i].links.CategoryUrl),
                },
                "isDeal":isDeal,
                "isOffer":isOffer,
                "Category":Category,
              ...
              console.log("->extract is ",extract," extractdata is ",extractData)
              await driver.sleep(1000);
              if(extract == "Excel" && extractData=="categorylinks"){
                console.log("=>Entered into app category links")
                var app = {
                  // "name":"firebase check",
                  // "id":"11"
                "oldid": id,
                "id": keyValueCounts[productCategory],
                "idlen": 10,
                "idlength": 10,
                // "//" date: new date(),
                "date": String(todayDate),
                ...
                "photo": String(photo),
                // "//" text: text,
                "producttext": String(urltext),
                "urltext": String(Title),
                "promotionText": String(PromotionText),
                "productType": "Affiliate",
                "storeType": String(itemType),
                "productCode": String(asin),
                "price": String(pricenum),
                ...
                console.log("app is ",app)
              ...
              else if(extract == "Excel" && extractData=="productlinks"){
                  var app = {
                    // "name":"firebase check",
                    // "id":"11"
                  "oldid": id,
                  ...
                  "dealLandingPage":dealLandingPage,
                  "asinData":asinData,
                  ...
                  "asinRating":asinRating,
                  "asinReviews":asinReviews,
              ...
              //   jsonFileName = productCategory
              // }
              // if((photo != "" && urlvalue1.length != 0 && urlvalue1!="" && (pricenum > 0 || (extract=="Excel" && (extractData=="productlinks" || extractData=="genericlinks"))) && (disco > 0 || (extract=="Excel" && (extractData=="productlinks" || extractData=="genericlinks")))) || (extractData == "categorylinks")){
              if((photo != "" && urlvalue1.length != 0 && urlvalue1!="" && (pricenum > 0 || (extract=="Excel" && (extractData=="productlinks" || extractData=="genericlinks"))) && ( (extract=="Excel" && (extractData=="productlinks" || extractData=="genericlinks")))) || (extractData == "categorylinks")){
              // if((photo != "" && pricenum > 0 && disco > 0 && urlvalue1.length != 0 && urlvalue1!="") || (extractData == "categorylinks")){
                if(enableFirebasePost){
                  

// Iterate through the input data
// inputData.forEach(key => {
  // Check if the key already exists in the keyValueCounts object
  // if (keyValueCounts.hasOwnProperty(productCategory)) {
  //   // If it exists, increment the count
  //   keyValueCounts[productCategory]++;
  // } else {
...
// Display the final key-value counts
// console.log("key value counts of ",productCategory, ",is ,",keyValueCounts[productCategory]);
                  postflag = await firebasepost(app,access_token,env,jsonFileName,url,extract,extractData);
                  console.log("Post Flag is ",postflag)
                  if(postflag){
                    id+=1;
                  } else {
                ...
                console.log("Details missing price is",price,"dicos is",disco,"photo is ",photo)
              }
              // firebase(app);
              //Need to be changed
            } catch (e) {
  ...
  try {
    ...
    await driver.get(`https://web.whatsapp.com/send?phone=9866017750&text=hidone\ndone`);
    // await driver.actions()
  }
}
